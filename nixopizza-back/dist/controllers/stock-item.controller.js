...
const getAllStockItems = async (req, res) => {
    try {
        const { location, product, stock, minQuantity, maxQuantity, createdAtFrom, createdAtTo, expireAtFrom, expireAtTo, sortBy, order, category, page = 1, limit = 10 } = req.query;
        ...
        // Filter by product
        if (product) {
            query.product = new mongoose_1.default.Types.ObjectId(product);
        }
        // Filter by category
        if (category) {
            const categoryProducts = await product_model_1.default.find({ categoryId: category }).select("_id");
        ...
        // Filter by stock
        if (stock) {
            query.stock = new mongoose_1.default.Types.ObjectId(stock);
        }
        ...
        if (location) {
            pipeline.push({
                $match: {
                    "stock.location": { $regex: location, $options: "i" }
                }
            });
        }
        ...
        const countResult = await stock_item_model_1.default.aggregate(countPipeline);
        const total = countResult.length > 0 ? countResult[0].total : 0;
        // Apply pagination
        pipeline.push({ $skip: skip });
        pipeline.push({ $limit: Number(limit) });
        const stockItems = await stock_item_model_1.default.aggregate(pipeline);
        res.status(200).json({
            total,
